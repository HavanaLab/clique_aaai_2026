...
from model import ScatteringNoFeaturesModel as Model
...
def least_probable_removal_numpy(
    ...
    probs_quantiles_first = np.digitize(
        subgraph_probs_first, bins=np.quantile(subgraph_probs_first, [0.25, 0.5, 0.75]), right=False
    )
...
datasets = [
    # "collab", # last runned instance has unmask problem
    # "imdb_binary",
    # "twitter",
    # "com-youtube",
    # "com-orkut",
...
for model_name in reversed([
        ...
        #  "collab_original","twitter_original","imdb_binary_original",
        "n1000_medium_instances_15_epochs_wc",
        # "com-youtube_original",
        # "com-orkut_original",
    ]):
    model_path = f"../models/scattering/{model_name}.pth"
    model = Model(model_path, device="cpu")
    model.model.eval()
    ...
    for dataset_name in datasets:
        ...
        # output_path = f"results/scattering ldr first 10 iteration quantiles clique/{model_name}_on_{dataset_name}.jsonl"
        output_base = "../results/scattering ldr first 10 iteration quantiles clique"
        ...
        with open(output_path, "a", 1) as f:
            for i, instance in enumerate(dataset):
                ...
                G_nx = nx.from_scipy_sparse_array(sparse_similarity_matrix)
                ...
                for layer in range(4):
                    for iteration, pc_quantiles in enumerate(
                        ...
                        unmasked_pc_quantiles = unmask(clique_members_mask, pc_quantiles)
                ...
                for iteration, degree_quantile in enumerate(history["degrees_quantiles"]):
                    unmasked_degree_quantile = unmask(clique_members_mask, degree_quantile)
                    non_clique_size = len(G) - clique_size
                    number_of_clique_members_in_q0 = sum(
                ...
                for iteration, probs_quantile in enumerate(history["probs_quantiles"]):
                    unmasked_probs_quantile = unmask(clique_members_mask, probs_quantile)
                    non_clique_size = len(G) - clique_size
                    number_of_clique_members_in_q0 = sum(
