...
def lpr(G):
    res = least_probable_removal(G, get_degrees, compute_once=True)
    return len(res)
...
def main():
    for di, dataset_name in enumerate(datasets_paths):
        for mi, model_name in enumerate(model_paths):
            ...
            output_base = "../results/diameter/topk"
            output_path = os.path.join(output_base, f"topk_on_{dataset_name}_eval_results.jsonl")
            os.makedirs(output_base, exist_ok=True)
            evals = [
                Evaluate(
                    lambda G: lpr(G), f"topk_degrees"
                )
            ]
...
def process_directory(directory, key="topk_degrees"):
...
if __name__ == "__main__":
    ...
    directory = "../results/diameter/topk"
